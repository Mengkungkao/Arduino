...
int acc_calibration_value = -699;                            //Enter the accelerometer calibration value
...
void loop() {
  ...
  angle_gyro = angle_gyro * 0.9996 + angle_acc * 0.0004;                    //Correct the drift of the gyro angle with the accelerometer angle//After 100 milliseconds the received byte is deleted
  ...
  if(angle_gyro < 30 && angle_gyro > 2){
    motor1.run(FORWARD);
    motor2.run(FORWARD);
    motor1.setSpeed(pid_output);
    motor2.setSpeed(pid_output);  
  }
 else if (angle_gyro > -30 && angle_gyro < -2) {
    motor1.run(BACKWARD);
    motor2.run(BACKWARD);
    motor1.setSpeed(pid_output);
    motor2.setSpeed(pid_output);  
